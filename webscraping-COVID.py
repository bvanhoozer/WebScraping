...
# Read all contents of the webpage
webpage = urlopen(req).read()

# Breaks up the webpage based on html tags (parser breaks up the file, Example: commas)
soup = BeautifulSoup(webpage,'html.parser')

# If you get the title printed back that means the website is scrapeable
print(soup.title.text)

# Search through all the rows in the webpage
...
for row in table_rows[2:53]:
    td=row.findAll("td")
    # 'Text' to only get the text portion of state in the second td/cell index
    state=td[1].text.rstrip('\n')
    total_cases=int(td[2].text.replace(",","")) # Replace comma with nothing
